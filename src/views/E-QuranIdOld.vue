        ...
        <div class="p-4 md:ml-52 md:mr-20 xlc:ml-72 xlc:mr-40">
            ...
            <div id="container-1" class="grid grid-cols-1 gap-4 md:px-8 md:pt-8 md:pb-16 smc:pb-28 smc:px-4 smc:pt-4">
                <div class="text-4xl text-right font-arab text-slate-700 sticky" v-for="surah in quran.ayahs" :key="surah.number">
                    ...
                    <h3 class="text-left smc:text-xl md:text-2xl m-5">{{ surah.latin }}</h3>
                    <audio id="sound-number" preload="metadata" controls class="w-1/2 bg-gray-200 p-4 rounded-full">
                      <source id="source-number" :src="'https://cdn.islamic.network/quran/audio/128/ar.alafasy/'+ surah.number + '.mp3'" type="audio/mpeg">
...
export default {
  ...
  async mounted() {
    ...
    setTimeout(() => {
      ...
      let audioNumber = document.querySelector('#sound-number')
